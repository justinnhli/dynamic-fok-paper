...
% unsure if this is correct/ necessary
$AbsoluteActivationFOK(s) = \sum {t_i^{-d}}$

\subsection{Relative Activation FOK}
$AvgActivationOfEverything= \frac {\sum\limits_{s: <s, p, o> \in E} {A(s)}}{|O|}$

\noindent$$RelativeCueActivation(s) =
\begin{cases}
				0 & {if AvgActivationOfEverything = 0} \\
				\ln(\frac{A(s)}{AvgActivationOfEverything}) & {if AvgActivationOfEverything \neq 0} \\
\end{cases}
$$

\noindent$$RelativeTargetActivation(s) =
\begin{cases}
				0 & {if AvgActivationOfEverything = 0} \\
				\exists o{\in}O, <s, p, o>{\in}E, \ln(\frac{A(o)}{AvgActivationOfEverything}) & {if AvgActivationOfEverything \neq 0, A(o) \neq 0} \\
\end{cases}
$$

\noindent$$RelativeActivationFOK(s) =
\begin{cases}
				RelativeCueActivation(s) & {if |Q| > 0} \\
				RelativeTargetActivation(s)& {if |Q| = 0} \\
\end{cases}
$$
